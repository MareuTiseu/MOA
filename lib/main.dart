...
class _MyAppState extends State<MyApp> {
  ...
  Widget todoListScreen() {
    ...
    //                                 height: 1.50,),);
    //                       } else {
    //                         return Text(snapshot.data![index].task,
    //                               overflow: TextOverflow.ellipsis,  // 글자수 넘어가면 ...으로 보여주는 속성
    //                               maxLines: 2,
    //                               style: TextStyle(
    //                                 fontSize: 16,
        ...
        onKey: (event) {
          if (event.isKeyPressed(LogicalKeyboardKey.enter)) {
            setState(() {
              ...
              //     birthday: now_date,
              //     category: 'total');
              // print (newTodoItem.task);
              // print (newTodoItem.birthday);
              // print (newTodoItem.status);
              // print (newTodoItem.category);
          ...
          children: [
            Wrap(  // 타이틀과 날짜를 묶어줌
                  ...
                  children: [
                    Image.asset('assets/MOA_title.png', height: 16,),
                    Text(
                      '${DateFormat('M').format(date)}월 ${DateFormat('d').format(date)}일 오늘의 To do',
                      style: const TextStyle(
                        color: Color(0xFF79747E),
                        fontSize: 16,
                        fontFamily: 'Noto Sans KR',
...
class _todoListScreenState extends State<todoListScreen> {
  ...
  void initState() {
    ...
    // 실행하면 가장 먼저 할 일 : 투두 전체를 포함할 CategoryList 하나 만들기
    setState(() {
      totalList.add("잘 나오나 확인용");
      // print(categoryList);
    });
  }
  ...
  @override
  Widget build(BuildContext context) {
    return todoCategoryListView(context);
  }

  Widget todoCategoryListView(BuildContext context) {
    return ListView.builder(
      /*
      아래 두 줄이 빠져서 이틀동안 고생함
      ListView 사용 후 'Vertical viewport was given unbounded height' 에러 뜰 시 확인하기
